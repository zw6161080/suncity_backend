Rails.application.configure do
  ...
  if ENV["EMAIL_SMTP_USERNAME"].present?
    config.action_mailer.smtp_settings[:user_name] = ENV["EMAIL_SMTP_USERNAME"]
    config.action_mailer.smtp_settings[:password] = ENV["EMAIL_SMTP_PASSWORD"]
    config.action_mailer.smtp_settings[:authentication] = 'plain'
  else
    config.action_mailer.default_options = { from: 'hris@suncity-group.com' }
  end
...
Rails.application.config.action_cable.allowed_request_origins = [FRONTEND_URL, "http://localhost:8989"]
