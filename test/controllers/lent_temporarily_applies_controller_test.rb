...
class LentTemporarilyAppliesControllerTest < ActionDispatch::IntegrationTest

  setup do
    ApplicationController.any_instance.stubs(:authorize).returns(true)
  end

  test "post create lent temporarily apply & show detail" do
    ...
    LentTemporarilyAppliesController.any_instance.stubs(:current_user).returns(user)
      ...
      lent_temporarily_items: [
        {
          region: 'macau',
          user_id: user.id,
          lent_date: '2017/01/10',
          return_date: '2017/02/10',
          lent_location_id: location.id,

          lent_salary_calculation: 'do_not_adjust_the_salary',
          return_salary_calculation: 'do_not_adjust_the_salary',
          comment: 'test comment 1',
        },
        {
          region: 'macau',
          user_id: user.id,
          lent_date: '2017/02/10',
          return_date: '2017/03/10',
          lent_location_id: location.id,
          lent_salary_calculation: 'do_not_adjust_the_salary',
          return_salary_calculation: 'do_not_adjust_the_salary',
    ...
    assert_difference(['LentTemporarilyApply.count'], 1) do
      assert_difference(['JobTransfer.count'], 2) do
        assert_difference(['ApprovalItem.count'], 3) do
          assert_difference(['LentTemporarilyItem.count'], 2) do
            assert_difference(['AttendAttachment.count'], 3) do
              assert_difference(['LentRecord.count'], 2) do
                post '/job_transfers/lent_temporarily_applies/can_create', params: params, as: :json
                assert_response :ok
                TimelineRecordService.stubs(:can_lent_record_create).with(any_parameters).returns(true)
                post '/job_transfers/lent_temporarily_applies', params: params, as: :json
                assert_response :ok
              end
            end
          end
    ...
    tta = LentTemporarilyApply.first
    get "/job_transfers/lent_temporarily_applies/#{tta.id}", as: :json
    assert_response :ok

    assert_equal 3, json_res['data']['approval_items'].count
    assert_equal 3, json_res['data']['attend_attachments'].count
    assert_equal 2, json_res['data']['lent_temporarily_items'].count
  end
end
