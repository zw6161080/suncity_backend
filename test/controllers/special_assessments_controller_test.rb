...
class SpecialAssessmentsControllerTest < ActionDispatch::IntegrationTest
  setup do
    ApplicationController.any_instance.stubs(:authorize).returns(true)
  end

  test "post create special_assessment & show special_assessment" do
    # user = create(:user)
    ...
    SpecialAssessmentsController.any_instance.stubs(:current_user).returns(user)
    params = {
      user_id: user.id,
      creator_id: user.id,
      region: 'macau',
    ...
    assert_difference(['SpecialAssessment.count'], 1) do
      assert_difference(['JobTransfer.count'], 1) do
        assert_difference(['ApprovalItem.count'], 3) do
          assert_difference(['AttendAttachment.count'], 3) do
            post '/job_transfers/special_assessments', params: params, as: :json
            spa = SpecialAssessment.first

            assert_response :ok
            assert_equal spa.assessment_questionnaire.items.count, 20
          end
        end
      end
    end
    ...
    spa = SpecialAssessment.first
    get "/job_transfers/special_assessments/#{spa.id}", as: :json
    assert_response :ok
