...
class EntryAppointmentsControllerTest < ActionDispatch::IntegrationTest

  setup do
    EntryAppointmentsController.any_instance.stubs(:current_user).returns(create_test_user)
    EntryAppointmentsController.any_instance.stubs(:authorize).returns(true)

  end

  test "should get index" do
    create(:user, id: 1)
    create(:entry_appointment)

    params_1 ={
      region: 'macau'
    }

    get '/entry_appointments', params: params_1
  ...
  test 'should export_xlsx' do
    EntryAppointmentsController.any_instance.stubs(:current_user).returns(User.first)
    params_2 ={
        region: 'macau'
    }

    get '/entry_appointments/export_xlsx', params: params_2
    assert_response :success
  end
  ...
  def test_cud
    ...
    qt = create(:questionnaire_template, template_type: 'entry_questionnaire')
    ...
    post entry_appointments_url, as: :json, params: entry_appointment_params
    assert_response :ok
    assert_equal json_res['data'], EntryAppointment.last.id
    assert_equal json_res['state'], 'success'

    ea = EntryAppointment.last

    get entry_appointment_url(ea.id)
    assert_response :ok
    ...
    get options_entry_appointments_url
    assert_response :ok
    ...
    patch entry_appointment_url(ea.id), as: :json, params: update_params
    assert_response :ok
    assert_equal EntryAppointment.find(json_res['data']).appointment_time, '13:00'

    get send_content_entry_appointment_url(ea.id)
    assert_response :ok
    assert_equal json_res['data']['sms'], '太陽城集團人力資源部短訊：恭喜閣下入職已滿3個月并通過了試用期！茲 通知閣下進行入職面談。
                   請於[2018年1月1日 13時0分],
                   前來1780面談，務必提前填寫 入職面談調查問卷 ，謝謝。
                   如有任何疑問，請於辦公時間內致電太陽城人力資源部： +853 8891 1332'
    assert_equal json_res['data']['email'], '恭喜閣下入職已滿3個月并通過了試用期！請閣下務必提前填寫 入職面談調查問卷 ，并準時進行入職面談。
                     面談時間：[2018年1月1日 13時0分]
                     電太陽城人力資源部： +853 8891 1332'


    assert_difference(['EntryAppointment.count'], -1) do
      delete entry_appointment_url(ea.id)
      assert_response :ok
    end
  end
end
