class TransferPositionApplyByDepartmentsController < ApplicationController
  ...
  def create
    ...
    ActiveRecord::Base.transaction do
      apply = TransferPositionApplyByDepartment.create(apply_params)
      ...
      apply.create_job_transfer(user,current_user,params[:new_career_record].permit(career_required_array + career_permitted_array),
                                ...
                                transfer_type: "transfer_position_apply_by_department",
  ...
  def show
    authorize TransferPositionApplyByDepartment
    apply = TransferPositionApplyByDepartment.find(params[:id])
  ...
  def create_career_record(user)
    ...
    new_row = new_row.merge inputer_id: current_user.id, user_id: user.id, deployment_type: 'transfer_by_department_initiated'
  ...
  def create_salary_record(user, apply)
    ...
    new_row = new_row.merge change_reason: 'transfer_by_department_initiated'
  ...
  def create_welfare_record(user, apply)
    ...
    new_row.merge(change_reason: 'transfer_by_department_initiated', welfare_begin: (params[:new_career_record]&.send(:[], :career_begin)|| params[:transfer_date]))
  ...
  def apply_params
    params.require(:transfer_position_apply_by_department).permit(
      :region,
      :user_id,
      :comment,
      :apply_date,
